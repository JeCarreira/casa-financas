...
function lsSave(){if(!LS_KEY)return;var s=JSON.stringify(getData());try{localStorage.setItem(LS_KEY,s);}catch(e){}try{sessionStorage.setItem(LS_KEY,s);}catch(e){}try{localStorage.setItem(LS_KEY+'_boca',JSON.stringify({data:bocaData,config:bocaConfig}));}catch(e){}try{localStorage.setItem(LS_KEY+'_renda',JSON.stringify(rendaData));}catch(e){}try{localStorage.setItem(LS_KEY+'_reservas',JSON.stringify(reservasData));}catch(e){}try{localStorage.setItem(LS_KEY+'_sajuste',String(saldoAjuste));}catch(e){}}
...
function renderTpl(){
  ...
  var html='<div style="font-size:12px;color:var(--t2);margin-bottom:.7rem;">Total do template: <strong>'+fmt(total)+'</strong> · Clica para editar · Usa <strong>Aplicar ao mês</strong> para copiar para as planeadas</div>';
...
function aplicarTpl(){
  ...
  var n=0;
  ativos.forEach(function(t){if(!despesas.some(function(d){return d.tplId===t.id&&mk(d.data)===m;})){var dia=String(t.dia||1).padStart(2,'0');despesas.push({id:uid(),tplId:t.id,desc:t.nome,valor:t.valor,cat:t.cat,data:m+'-'+dia,tipo:'planeada',pago:false});n++;}});
...
function renderDesp(){
  ...
  Object.entries(byCat).forEach(function(e){var cat=e[0],items=e[1],ct=items.reduce(function(s,d){return s+d.valor;},0);
    ...
    html+=items.map(function(d){return'<div class="li" style="'+(d.pago?'opacity:.5;':'')+'">'
      +'<div class="ll"><div class="ln" style="'+(d.pago?'text-decoration:line-through;':'')+'">'+d.desc+(d.projetada?'<span style="font-size:10px;color:var(--t3);margin-left:4px;">proj.</span>':'')+'</div><div class="ls">'+d.data+(d.projetada?' · proj.':'')+'</div></div>'
